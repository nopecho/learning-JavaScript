...
const findFriendsBySortCityAndAge = friends
    .sort((f1,f2) => f1.age - f2.age)
    .sort( (f1,f2) => f1.city < f2.city ? -1 : 1 )
    .filter(f => f.city === 'seoul')
 
console.log(findFriendsBySortCityAndAge)
...
// I am
// `
// const modifiedSentence = lyrics.split(' ')
//     .map(word => word.includes('shit') || word.includes('똥') ? '💩' : word )
//     .map(word => word.includes('fuck')? '❤️' : word)
//     .join(' ')
// console.log(modifiedSentence)
...
novel.split('')
    .filter(a => !specialCharacters.includes(a))
    .forEach(a => classified[a.toLowerCase()] = (++classified[a.toLowerCase()] || 1))
...
lyrics.split(/[ \n]/).filter(e => e!='').forEach(ly => displayLyric(bliudDom(ly)))

function displayLyric(lyric){
    lyricsEl.appendChild(lyric)
}

function bliudDom(lyric){
    const item = document.createElement('div')
    item.className = 'lyric'
    item.innerText = lyric
    return item;
}
...
function highLightWords(e){
    ...
    const lyric = document.querySelectorAll('.lyric')
    if(!isValid(search)){
        lyric.forEach(e => e.classList.remove('highlight'));
    }else{  
        lyric.forEach(e => {
            e.classList.remove('highlight')
            if(e.innerText.includes(search.toLowerCase())){
